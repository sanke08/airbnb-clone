...
const Counter = ({ onChange, value }) => {
    return (
        <div className=' space-x-4'>
            <Button variant={"none"} onClick={() => onChange(undefined)} className={twMerge(' border border-neutral-300 hover:border-black px-3 sm:px-4', value == undefined && "bg-black text-white")}>Any</Button>
            {
                [...Array(5)].map((i, j) => (
                    <Button variant={"none"} onClick={() => onChange(j + 1)} className={twMerge(' border border-neutral-300 hover:border-black px-3 sm:px-4', value === j + 1 && "bg-black text-white")} key={j}>{j + 1} </Button>
                ))
            }
        </div>
    )
