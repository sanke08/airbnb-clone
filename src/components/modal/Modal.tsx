...
const Modal = ({ children, open, close, className, header, description }: Props) => {
    ...
    return (
        <div className={twMerge(' h-screen w-screen flex justify-center items-center top-0 left-0 z-50 bg-black/50 ', open ? "fixed" : "hidden")}>
            <div className={twMerge(' w-[35rem] min-h-72 max-h-full px-5 bg-white border-2 rounded-xl shadow-xl relative transition-all duration-500', className, isOpen ? " translate-y-[0%] opacity-100" : " opacity-0 translate-y-[50%]")}>
